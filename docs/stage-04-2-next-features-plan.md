# Stage 04-2: 图谱可视化 · 会话持久化 · Prompt 优化（Deprecated）

## 状态说明

本文档已废弃，不再作为当前工作区的实施基线。

废弃原因：

- 本文档编写于 `stage-04-3` 多场景泛化之前，部分数据模型和接口签名已变化
- Prompt 体系已在 `stage-04-4` / `stage-04-5` 中经历多轮重构，本文档中的 Prompt 规划不再对应当前实现
- 会话持久化设计依赖旧版会话模型，未覆盖当前 `scenario_id / scenario_label / dataset_name` 等字段，也未覆盖 agent 当前的多次增量落盘时机

替代关系：

- 会话持久化：见 `stage-04-6`
- Prompt 当前实现基线：见 `stage-04-5`
- 图谱可视化：后续如继续推进，应基于当前前后端代码状态重新单列规划

阅读建议：

- 可以将本文档视为历史归档，保留当时的思路和 feature 拆分
- 不应再直接依据本文档执行开发
